...
def test_compress_large_image_causes_memory_error(client):
    ...
    # Expected: Should handle large files gracefully with 413 status
    # Actual: Will raise MemoryError or return 500
    assert response.status_code == 413 or response.status_code == 400
    
    # If it doesn't crash, it should return an error message
...
@pytest.mark.xfail(reason="KNOWN ISSUE: No file size validation before processing", strict=False)
def test_compress_extremely_large_image_validation(client):
    ...
    # Expected: Should validate and reject with 413 Payload Too Large
    # Actual: May crash with MemoryError before validation
